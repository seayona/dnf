...
class SouthWorker(QThread):
    # 定义一个信号
    trigger = pyqtSignal(str)

    def __init__(self, voyager):
        # 初始化函数，默认
        super(SouthWorker, self).__init__()
    ...
    def _run(self):
        ...
        print(self.voyager.recogbot.daily_south())
        # 发现南部入口
        if self.voyager.recogbot.daily_south() and not self.voyager.player.ticket():
            print("【南部】发现南部入口！")
            self.voyager.game.south_fight()
        ...
        if self.voyager.recogbot.daily_town() and not self.voyager.recogbot.replay():
            self.voyager.game.daily_town(8)

        if self.voyager.recogbot.daily_south_completed():
            print(f"【南部】南部已刷完！")
            self.voyager.player.over_south()
            self.voyager.game.back_town_daily()

        if self.voyager.recogbot.daily_confirm_grey() and self.voyager.recogbot.close():
            print(f"【南部】南部已刷完！不要在点了！")
            self.voyager.player.over_south()
            self.voyager.game.back_town_daily()

        if self.voyager.recogbot.town() and self.voyager.player.ticket():
